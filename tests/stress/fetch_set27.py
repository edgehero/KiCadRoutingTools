#!/usr/bin/env python3
"""Fetch set27 .kicad_pcb sources listed in manifest_set27.json (raw download).

Downloads each board (and its sibling .kicad_pro, which carries the DRC floor)
into $STRESS_DIR/sources/github_set27/. After fetching, run `bash prep_set27.sh`
(needs KiCad's bundled python / pcbnew) to produce boards_set27/ (routed
reference) + boards_unrouted_set27/ (stripped).

  python3 fetch_set27.py
  bash prep_set27.sh
...
MANIFEST = HERE / "manifest_set27.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set27"
    ...
    print(f"\n{ok}/{len(boards)} set27 sources -> {out_dir}")
